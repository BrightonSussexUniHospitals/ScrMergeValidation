...
SET ANSI_NULLS ON
GO
CREATE VIEW [Merge_DM_MatchViews].[ltblDIAGNOSIS] AS
...
**************************************************************************************************************************************************/
	...
	SELECT  CAST(1 AS tinyint) AS SrcSysID
			,ID
			,DIAG_CODE = DIAG_CODE COLLATE DATABASE_DEFAULT
			,DIAG_DESC = DIAG_DESC COLLATE DATABASE_DEFAULT
			,HIGHLIGHT
			,REG_CONDITION
			,IS_DELETED
			,IS_DELETED_UGI
			,IsLeukaemiaCode
			,FasterDiagnosisCancerSiteID
			,IsTesticularCode
