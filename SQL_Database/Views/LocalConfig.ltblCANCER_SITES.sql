...
SET ANSI_NULLS ON
GO
CREATE VIEW [LocalConfig].[ltblCANCER_SITES] AS
...
**************************************************************************************************************************************************/
	...
	SELECT  CAST(1 AS tinyint) AS SrcSysID
			,CA_ID
			,CA_SITE = CA_SITE COLLATE DATABASE_DEFAULT
			,ROLE_NAME = ROLE_NAME COLLATE DATABASE_DEFAULT
			,HNATargetDays
			,TertiaryRequestedTargetDays
			,TertiaryReceivedTargetDays
			,IsCancerSite
	FROM  [CancerRegister_WSHT]..ltblCANCER_SITES
	...
	SELECT  CAST(2 AS tinyint) AS SrcSysID
			...
			,TertiaryReceivedTargetDays
			,IsCancerSite
	FROM  [CancerRegister_BSUH]..ltblCANCER_SITES
GO
